# ⚡ IQ Option 24/7 Live REST API & Signal Engine

> **Production-grade 24/7 self-healing REST API & WebSocket wrapper for IQ Option (Real & OTC asset pairs).**
...
- 🕒 **24/7 Real & OTC Coverage**: Real-time market data streaming for IQ Option.
...
| `/api/iq option/live-price` | `GET` | Real-time price stream & tick updates |
| `/api/iq option/candles` | `GET` | Closed M1/M5 historical candle datasets |
| `/api/iq option/payouts` | `GET` | Live payout percentage monitor across all pairs |
| `/api/iq option/signals` | `POST` | Strategy webhook trigger & alert dispatcher |
